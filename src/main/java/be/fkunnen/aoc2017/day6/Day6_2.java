...
public class Day6_2 {
    ...
    public int memoryReallocation(String input) {
        ...
        boolean isInfiniteLoopDetected = false;
        List<Integer> configurationToBeSeenAgain = new ArrayList<>();
        ...
        while (keepGoing){
            ...
            if (isInfiniteLoopDetected && configurationToBeSeenAgain.equals(reallocatedMemoryBanks)){
                keepGoing = false;
            }
            if (!isInfiniteLoopDetected && isInfiniteLoopDetected(reallocatedMemoryBanks, seenConfigurationsAfterReallocation)){
                isInfiniteLoopDetected = true;
                redistributionCycles = -1;
                configurationToBeSeenAgain.addAll(reallocatedMemoryBanks);
            }
